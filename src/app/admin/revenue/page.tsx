...
export default function RevenuePage() {
    ...
    // ---- Compute real revenue gaps from live data ----
    const opportunities = useMemo<Opp[]>(() => {
        ...
        // 1) Underused suite — value the empty seats
        ...
        if (worst) {
            ...
            if (upside > 0) out.push({
                id: 'gap-occupancy',
                action: 'Fill empty seats',
                title: `${worst.s.name} runs light`,
        ...
        // 3) Tier-upgrade cohort — Associates booking like Suite members
        ...
        if (cohort.length > 0) {
            const upside = round(cohort.length * 1000 * 12 * 0.5); // $1k/mo uplift, 12mo, 50% convert
            out.push({
                id: 'gap-upgrade',
                action: 'Tier upgrade outreach',
                title: 'Associates booking like Suite members',
        ...
        // 4) At-risk revenue — flagged members who still spend
        ...
        if (atRisk.length > 0 && atRiskRevenue > 0) {
            out.push({
                id: 'gap-retention',
                action: 'Protect at-risk revenue',
                title: 'Revenue tied to flagged members',
    ...
    const act = (id: string, kind: 'approve' | 'dismiss') => {
        ...
        addAuditEntry(
            op,
            kind === 'approve' ? 'APPROVE_OPPORTUNITY' : 'DISMISS_OPPORTUNITY',
            id,
            ...
            {opportunities.length === 0 ? (
                <Card className="glass border-border-muted">
                    <CardContent className="py-12 text-center text-muted-foreground text-sm">
                        No revenue gaps detected in the current data. As bookings and members change, opportunities surface here.
                    </CardContent>
                </Card>
            ) : (
