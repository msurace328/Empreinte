...
class DataService {
    ...
    async updateApplicationStatus(appId: string, status: Application['status'], operator: string, reason: string) {
        ...
        if (app) {
            app.status = status;
            this.addAuditEntry(operator, `${status}_APPLICATION`, appId, reason);

            // If approved, create a member
            if (status === 'Approved') {
    ...
    async updateMemberStatus(memberId: string, status: Member['status'], operator: string, reason: string) {
        const member = this.members.find(m => m.id === memberId);
        if (member) {
            member.status = status;
            this.addAuditEntry(operator, `${status}_MEMBER`, memberId, reason);
        }
    }

    async addAuditEntry(operator: string, action: string, targetId: string, reason: string) {
        ...
        const newEntry: AuditEntry = {
            ...
            targetId,
            reason,
            hash: Math.random().toString(36).substring(2, 10), // Mock hash
            previousHash: lastEntry?.hash || '000000',
        };
